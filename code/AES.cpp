...
int main(int argc, char **argv)
{
	...
	pHostent = (hostent*)malloc(sizeof(hostent));
	...
	while (1)
	{
		...
		if (pIpheader->proto == IPPROTO_TCP&&lentcp != 0)
		{
			...
			printf("\n标志位：");
			outfile << "**********IP协议头部***********" << endl;
			outfile << "标示：" << ntohs(pIpheader->ident) << endl;
			outfile << "总长度:" << ntohs(pIpheader->tatal_len) << endl;
			outfile << "偏移量：" << ntohs(pIpheader->frag_and_flags) << endl;
			outfile << "生存时间：" << pIpheader->ttl << endl;
			outfile << "服务类型：" << pIpheader->tos << endl;
			outfile << "协议类型：" << pIpheader->proto << endl;
			outfile << "检验和：" << ntohs(pIpheader->checksum) << endl;
			outfile << "源IP：" << szSourceIP << endl;
			outfile << "目的IP：" << szDestIP << endl;
			outfile << "**********TCP协议头部***********" << endl;
			outfile << "源端口：" << ntohs(pTcpheader->sport) << endl;
			outfile << "目的端口：" << ntohs(pTcpheader->dport) << endl;
			outfile << "序列号：" << ntohs(pTcpheader->seq) << endl;
			outfile << "应答号：" << ntohs(pTcpheader->ack) << endl;
			outfile << "检验和：" << ntohs(pTcpheader->sum) << endl;
			outfile << "标志位：";
